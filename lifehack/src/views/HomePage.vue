  ...
  beforeMount() {
    ...
    onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        const q = query(docRef2, where("userID", "==", user.uid));
        ...
        docRef.then(function (snapshot) {
          ...
          buddyArray.forEach((buddy) => {
            ...
            vm.upcomingVisits.push(
              buddy.visitDate.toDateString() +
                " " +
                zero +
                buddy.visitDate.getHours() +
                "00  - " +
                buddy.endDate.getHours() +
                "00: Visit " +
                buddy.name
            );
          });
        ...
        querySnapshot.then(function (reminderArray) {
          reminderArray.forEach((r) => {
            vm.reminders.push(
              new Date(r.data().date).toDateString() +
                " : " +
                r.data().reminder +
                " (" +
                r.data().buddyName +
